class PastoralManager {
    constructor() {
        ...
        if (this.grupoAtual) {
            this.loadGrupoData().then(() => this.showApp());
        } else {
    ...
    async loadGrupoData() {
        ...
        try {
            this.casaisPastoral = await supaDB.getCasaisPastoral(this.grupoAtual);
            this.familiasSorteadas = await supaDB.getFamiliasSorteadas(this.grupoAtual);
            this.escala = await supaDB.getEscala(this.grupoAtual);
            this.tercos = await supaDB.getTercos(this.grupoAtual);
    ...
    async selecionarGrupo(grupo) {
        ...
        await this.loadGrupoData();
        await this.carregarExemplosGrupo();
        this.showApp();
    }
    ...
    async carregarExemplosGrupo() {
        if (this.casaisPastoral.length === 0) {
            const dadosGrupos = this.getDadosGrupo();
            this.casaisPastoral = dadosGrupos.casais;
            for (const casal of this.casaisPastoral) {
                try {
                    await supaDB.addCasalPastoral(this.grupoAtual, casal);
                } catch (error) {
                    console.warn('Casal já existe ou erro ao inserir:', error);
                }
            }
        }
    }
    ...
    showCasalPastoralModal(casal = null) {
        ...
        document.getElementById('casalForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            ...
            try {
                if (isEdit) {
                    await supaDB.updateCasalPastoral(casal.id, casalData);
                    const index = this.casaisPastoral.findIndex(c => c.id === casal.id);
                    this.casaisPastoral[index] = casalData;
                } else {
                    const novoCasal = await supaDB.addCasalPastoral(this.grupoAtual, casalData);
                    this.casaisPastoral.push(novoCasal);
                }
                this.closeModal();
                this.render();
            } catch (error) {
    ...
    async deleteCasalPastoral(id) {
        if (confirm('Excluir este casal da pastoral?')) {
            try {
                await supaDB.deleteCasalPastoral(id);
